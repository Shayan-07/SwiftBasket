...
const ProductCard = ({ product }) => {
  ...
  return (
    <div
      className='productCard productItem overflow-hidden rounded-3xl shadow-[4px_4px_8px_rgba(0,0,0,0.13)] group !w-full'
      onMouseEnter={() => setIsHover(true)}
      onMouseLeave={() => setIsHover(false)}
    >
      <div className="productImg h-[20rem] overflow-hidden relative" >
        <Link to={`/productdetails/${product._id}`} >
          <img
            src={product.mediaImgUrls[0]}
            alt={product.title}
            className={`w-full h-full object-cover object-center absolute transition-all duration-500 ${isHover ? 'opacity-0 scale-110' : 'opacity-100 scale-100'}`}
          />
          <img
            src={product.mediaImgUrls[1]}
            alt={product.title}
            className={`w-full h-full object-cover object-center absolute transition-all duration-500 ${isHover ? 'opacity-100 scale-100' : 'opacity-0 scale-110'}`}
          />
        </Link>
      ...
      <div className="productDetails text-[#3e3e3e] flex flex-col gap-3 px-3 rounded-b-3xl py-6">
        <Link className="productBrand text-[1.3rem] font-normal primary-hov truncate capitalize" to={`/productdetails/${product._id}`}>
          {product.brandName}
        </Link>
        <Link to={`/productdetails/${product._id}`} className="productName text-[1.5rem] font-medium text-[#222] primary-hov truncate capitalize">
          {product.title}
        </Link>
        <Rating name="read-only size-large half-rating" value={product.totalRating} precision={0.5} size='large' readOnly />
        <ul className="productPrice flex justify-between text-[1.6rem] font-[600]">
          <li className="oldPrice line-through opacity-70">${product.price}</li>
