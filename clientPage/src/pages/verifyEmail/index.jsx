...
const VerifyEmail = () => {
    ...
    const [isCoolDown, setIsCoolDown] = useState(parseInt(localStorage.getItem('EmailCoolDown')) || 0)
    ...
    const resend = () => {
        ...
        if (email) {
            if (isCoolDown === 0) {
                setIsResending(true)
                fetch(`${appURI}/auth/verifyemail/resendotp`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    ...
                    .then(resData => {
                        if (resData.success) {
                            ...
                            localStorage.setItem('EmailCoolDown', resData.coolDown)
                            setIsCoolDown(parseInt(localStorage.getItem('EmailCoolDown')))
                        } else {
                            setIsResending(false)
                            setIsError(resData.message)
                        }
        ...
        if (isCoolDown > 0) {
            const timer = setTimeout(() => {
                setIsCoolDown(prev => {
                    ...
                    localStorage.setItem('EmailCoolDown', newValue)
                    return newValue
                })
            }, 1000)
